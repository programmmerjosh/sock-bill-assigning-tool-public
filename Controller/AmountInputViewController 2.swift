...
class AmountInputViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func infoButton(_ sender: UIBarButtonItem) {
        let alert       = UIAlertController(title: "Info", message: "1. At your table, find out who all need to pay a portion of the bill.\n\n2. Press the '+' button to add each payer's name, one by one.\n\n~A textbox will appear once you have added at least one name.\n\n3. Now, go through the bill item by item, inserting each item's price into the textbox and tapping on the person's name that will be paying for that particular item.\n\n4. Once every item's price has been assigned to a payer, tap the 'Next' button to see the breakdown.", preferredStyle: .alert)
    ...
    func addNewNameButton(_ title: String) {
        ...
        if (buttonY + topMarginHeight + buttonHeightAndPadding) > Int(self.view.frame.height - keyboardHeight - CGFloat(topMarginHeight)) {
            myScrollView.contentSize = CGSize(width: self.view.frame.width, height: CGFloat(buttonY + topMarginHeight) + keyboardHeight)
        }
    }
